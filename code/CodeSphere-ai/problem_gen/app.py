...
# 환경 변수 로드
load_dotenv("/home/graduate/CodeSphere-ai/problem_gen/.env")
...
# 메인 실행 로직
def generate_problem_once(algorithm_type: str, difficulty: str):
...
"""
    ...
    return {
        ...
        "problem_input_desc": problem_input_desc,
        "problem_output_desc": problem_output_desc,
        "problem_constraints": problem_constraints,
        "solution_code": solution_code,
        "example_io": [{"input": i, "output": o} for i, o in public_examples],
        "test_io": [{"input": i, "output": o} for i, o in grading_cases]
    }
...
def api_generate_problem():
    ...
    algorithm_type = payload.get("algorithm_type", "DFS")
    difficulty = payload.get("difficulty", "중간")

    try:
        result = generate_problem_once(algorithm_type, difficulty)
        return jsonify({"ok": True, **result})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
    
if __name__ == "__main__":
    app.run(host="127.0.0.1", port=7043)